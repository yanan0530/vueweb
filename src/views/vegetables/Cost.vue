<template>
	<div>
		...
		<el-table :data="list" border>
			<el-table-column label="支出类型" prop="cost_type_name">
			</el-table-column>
			<el-table-column label="支出金额" prop="moneys">
			</el-table-column>
		...
		<el-dialog title="添加/编辑" :visible.sync="visi" width="500px">
			<el-form :model="saveForm" label-width="80px">
				<el-form-item label="支付类型">
					<el-select v-model="saveForm.cost_type">
						<cost-type-dict></cost-type-dict>
					</el-select>
				</el-form-item>
...
<script>
	...
	import CostTypeDict  from "@/components/dict/CostTypeDict.vue"
	export default {
		components: {
			xPag,CostTypeDict
		},
		...
		methods: {
			getList() {
				costListApi(this.page).then(res => {
					let data = res.data
			...
			saveClick() {
				...
				let saveApi
				if (data.id) {
					saveApi = costUpdateApi(data)
				} else {
					saveApi = costCreateApi(data)
				}
				saveApi.then(res => {

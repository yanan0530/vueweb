<template>
	<div>
		...
		<el-table :data="list" border>
			<el-table-column label="支出类型" prop="name">
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
				costtypeListApi(this.page).then(res => {
					let data = res.data
			...
			saveClick() {
				...
				let saveApi
				if (data.id) {
					saveApi = costtypeUpdateApi(data)
				} else {
					saveApi = costtypeCreateApi(data)
				}
				saveApi.then(res => {

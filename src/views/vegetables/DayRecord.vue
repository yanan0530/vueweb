<template>
	<div>
		...
		<el-table :data="list" border>
			<el-table-column label="工作类型" prop="job_type_name">
			</el-table-column>
		...
		<el-dialog title="添加/编辑" :visible.sync="visi" width="500px">
			<el-form :model="saveForm" label-width="80px">
				...
				<el-form-item label="内容">
					<el-select v-model="saveForm.job_type">
						<job-type-dict></job-type-dict>
					</el-select>
				</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
				dayrecordListApi(this.page).then(res => {
					let data = res.data
			...
			saveClick() {
				...
				let saveApi
				if (data.id) {
					saveApi = dayrecordUpdateApi(data)
				} else {
					saveApi = dayrecordCreateApi(data)
				}
				saveApi.then(res => {
			...
			delClick(id) {
				this.$confirm("确定要删除当前蔬菜分类么?").then(() => {
					dayrecordDeleteApi(id).then(res => {

<template>
	<div>
		...
		<el-table :data="list" border>
			<el-table-column label="工作" prop="name">
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
				jobtypeListApi(this.page).then(res => {
					let data = res.data
					console.info(data)
			...
			saveClick() {
				...
				let saveApi
				if (data.id) {
					saveApi = jobtypeUpdateApi(data)
				} else {
					saveApi = jobtypeCreateApi(data)
				}
				saveApi.then(res => {
